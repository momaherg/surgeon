"""
Enhanced interactive script for NPT permanent updates with configurable strength.
"""
...
# Import the strong update functions
sys.path.append(os.path.dirname(os.path.abspath(__file__)))
from improved_permanent_update import apply_permanent_update_strong, enhance_npt_for_stronger_updates


class EnhancedInteractiveTester:
    """Enhanced interactive tester with configurable update strength."""
    
    def __init__(self, checkpoint_path: str, modulation_scale: float = 0.5):
        self.checkpoint_path = checkpoint_path
        self.injected_facts = []
        self.test_results = []
        
        # Configuration for stronger updates
        self.default_alpha = 1.0  # Much stronger than original 0.1
        self.default_iterations = 2  # Apply update multiple times
        ...
        self.model = enhance_npt_for_stronger_updates(self.model, self.modulation_scale)
    ...
    def load_npt_checkpoint(self):
        ...
        print(f"Enhanced with modulation scale: {self.modulation_scale}\n")
        
        return model, tokenizer
    
    def inject_fact(self, fact: str, alpha: float = None, iterations: int = None, 
                   layers: List[int] = None) -> Dict:
        """Inject a fact with configurable strength."""
        ...
        print(f"  - Layers: {f'{len(layers)} specific layers' if layers else 'all layers'}")
        print("=" * 80)
    ...
    def interactive_mode(self):
        ...
        print("\n" + "="*80)
        print("NPT PERMANENT UPDATE - ENHANCED INTERACTIVE MODE")
        print("="*80)
        print(f"\nDefault configuration:")
        print(f"  - Modulation scale: {self.modulation_scale} (original: 0.1)")
        print(f"  - Default alpha: {self.default_alpha} (original: 0.1)")
        print(f"  - Default iterations: {self.default_iterations}")
        print("\nCommands:")
        print("  inject <fact>       - Inject a fact with default settings")
        print("  inject+ <fact>      - Inject with custom settings (interactive)")
        print("  test <prompt>       - Test model response")
        print("  recall              - Test recall of all injected facts")
        print("  general             - Test general knowledge retention")
        print("  config              - Show/change default configuration")
        ...
        while True:
            try:
                ...
                if command == 'quit' or command == 'exit':
                    ...
                    alpha_input = input(f"  Alpha (strength) [{self.default_alpha}]: ").strip()
                    alpha = float(alpha_input) if alpha_input else self.default_alpha
                    
                    iter_input = input(f"  Iterations [{self.default_iterations}]: ").strip()
                    iterations = int(iter_input) if iter_input else self.default_iterations
                    
                    layers_input = input("  Specific layers (comma-separated, or Enter for all): ").strip()
                    layers = None
                    if layers_input:
                        layers = [int(x.strip()) for x in layers_input.split(',')]
                    
                    self.inject_fact(fact, alpha=alpha, iterations=iterations, layers=layers)
                    ...
                    filename = f"npt_enhanced_session_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def test_injected_facts(self):
        ...
        for i, fact_record in enumerate(self.injected_facts, 1):
            ...
            print(f"   (alpha={fact_record['alpha']}, iter={fact_record['iterations']})")
    ...
    def show_config(self):
        """Show and optionally change configuration."""
        print("\n" + "="*80)
        print("CONFIGURATION")
        print("="*80)
        print(f"Current settings:")
        print(f"  - Modulation scale: {self.modulation_scale}")
        print(f"  - Default alpha: {self.default_alpha}")
        print(f"  - Default iterations: {self.default_iterations}")
        
        change = input("\nChange settings? (y/n): ").strip().lower()
        if change == 'y':
            alpha_input = input(f"New default alpha [{self.default_alpha}]: ").strip()
            if alpha_input:
                self.default_alpha = float(alpha_input)
                print(f"Default alpha set to: {self.default_alpha}")
            
            iter_input = input(f"New default iterations [{self.default_iterations}]: ").strip()
            if iter_input:
                self.default_iterations = int(iter_input)
                print(f"Default iterations set to: {self.default_iterations}")
    ...
    def show_status(self):
        ...
        for i, fact in enumerate(self.injected_facts, 1):
            print(f"  {i}. {fact['fact']} (α={fact['alpha']}, iter={fact['iterations']})")
...
def main():
    """Main entry point."""
    if len(sys.argv) < 2:
        print("Usage: python interactive_permanent_update_enhanced.py <checkpoint_path> [modulation_scale]")
        print("Example: python interactive_permanent_update_enhanced.py ./outputs/npt-improved-1B/checkpoint-500 0.5")
        sys.exit(1)
    
    checkpoint_path = sys.argv[1]
    modulation_scale = float(sys.argv[2]) if len(sys.argv) > 2 else 0.5
    
    try:
        tester = EnhancedInteractiveTester(checkpoint_path, modulation_scale)
